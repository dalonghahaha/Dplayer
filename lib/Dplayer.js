...
Dplayer.prototype._class = {
    ...
    VIDEO_MASK_ABOUT:'video-mask-about hidden',
...
Dplayer.prototype.build_video = function() {
    ...
    if (!this.config.multiple) {
        ...
        if(this.runtime.playlist[0].src instanceof Array){
            var video_src_index = this.get_video_src_index(this.runtime.playlist[0].src);
            this.runtime.video.src = this.runtime.playlist[0].src[video_src_index]['url'];
            this.runtime.play_definition = this.runtime.playlist[0].src[video_src_index]['index'];
        } else {
    ...
    //this.runtime.video_container.appendChild(this.create_element(this._class.VIDEO_MASK_ABOUT,'fuck you'));
...
Dplayer.prototype.build_video_mask = function() {
    ...
    var video_mask_about = this.create_element(this._class.VIDEO_MASK_ABOUT);
    ...
    var about_name = this.create_element(this._class.VIDEO_MASK_TEXT,'版本:1.0.0');
    about_name.setAttribute('action','http://www.Dplayer.com');
    video_mask_about.appendChild(about_name);

    var about_name = this.create_element(this._class.VIDEO_MASK_TEXT,'作者:龙翔云际');
    about_name.setAttribute('action','http://dalong.me');
    video_mask_about.appendChild(about_name);
    ...
    video_mask.appendChild(video_mask_about);
    return video_mask; 
}
...
Dplayer.prototype.build_progress_control_bar = function() {
    ...
    //进度调节器
    progress_bar.appendChild(this.create_element(this._class.KNOB_BAR));
...
Dplayer.prototype.rigester_video_mask_event = function(){
...
Dplayer.prototype.rigester_progress_event = function() {
    ...
    this.delegate(this.query_element('Dplayer'),'', 'mouseout', this.on_progress_off.bind(this));
    ...
    document.addEventListener('mouseup', function() {
        ...
        this.add_class(this.query_element('video-mask-about'),'hidden');
    }.bind(this));
}
...
Dplayer.prototype.play = function() {
    ...
    this.query_element("icon-switch").className = this._class.ICON_PAUSE;
}
...
Dplayer.prototype.pause = function() {
    ...
    this.query_element("icon-switch").className = this._class.ICON_PLAY;
}
...
Dplayer.prototype.on_video_mask_click = function(event){
    ...
    this.query_element('video-mask-about').style.left = offset_x + 'px';
    this.query_element('video-mask-about').style.top = offset_y + 'px';
    this.remove_class(this.query_element('video-mask-about'),'hidden');
    event.preventDefault();
}

Dplayer.prototype.on_video_about_click =function(event){
...
Dplayer.prototype.on_progress_on = function(event) {
    this.remove_class(this.query_element("knob-bar"), 'hidden');
...
Dplayer.prototype.on_progress_off = function(event) {
    ...
    this.add_class(this.query_element("knob-bar"), 'hidden');
}
