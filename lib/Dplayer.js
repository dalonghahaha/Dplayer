...
Dplayer.prototype.rigester_progress_event = function() {
    this.delegate(this.query_element('control-bar'),'', 'mousemove', this.on_progress_on.bind(this));
...
Dplayer.prototype.change_progress = function(position) {
    ...
    if (left <= knob_width / 2) {
        this.query_element("knob-bar").style.left = knob_width / 2;
    } else if (left >= (total_width - knob_width / 2)) {
        this.query_element("knob-bar").style.left = (total_width - knob_width / 2);
    } else {
